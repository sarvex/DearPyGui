...
def version_number():
    ...
    try:
        with open('version_number.txt', encoding='utf-8') as f:
            ...
            return "0.0.1" if (version.startswith("ECHO")) else version
    except IOError:
        return wip_version
  ...
  def run(self):
      ...
      if get_platform() == "Windows":
          command = [
              ...
              'cmake .. -G "Visual Studio 16 2019" -A "x64" -DMVDIST_ONLY=True -DMVDPG_VERSION=',
              f"{version_number()} -DMV_PY_VERSION=",
              f"{str(sys.version_info[0])}.{str(sys.version_info[1])} && ",
              "cd .. && cmake --build cmake-build-local --config Release",
          ]
          self.announce(
              'Running command: Dear PyGui Build for Windows',
              level=distutils.log.INFO,
          )
          subprocess.check_call(''.join(command), env=os.environ, shell=True)
          src_path = os.path.dirname(os.path.abspath(__file__))
          shutil.copy(
              "cmake-build-local/DearPyGui/Release/_dearpygui.pyd",
              f"{src_path}/output/dearpygui",
          )
          ...
          command = [
              "mkdir cmake-build-local; ",
              "cd cmake-build-local; ",
              f'cmake .. -DMVDIST_ONLY=True -DMVDPG_VERSION={version_number()} -DMV_PY_VERSION={str(sys.version_info[0])}.{str(sys.version_info[1])}; ',
              "cd ..; cmake --build cmake-build-local --config Release",
          ]
          self.announce(
              'Running command: Dear PyGui Build for Linux',
              level=distutils.log.INFO,
          )
          subprocess.check_call(''.join(command), shell=True)
          src_path = os.path.dirname(os.path.abspath(__file__))
          shutil.copy(
              "cmake-build-local/DearPyGui/_dearpygui.so",
              f"{src_path}/output/dearpygui",
          )
          ...
          self.announce(
              'Running command: Dear PyGui Build for OS X',
              level=distutils.log.INFO,
          )
          subprocess.check_call(''.join(command), shell=True)
          src_path = os.path.dirname(os.path.abspath(__file__))
...
def setup_package():
    ...
    if os.path.isdir(f"{src_path}/output"):
        shutil.rmtree(f"{src_path}/output")
    os.mkdir(f"{src_path}/output")
    os.mkdir(f"{src_path}/output/dearpygui")

    if os.path.isdir(f"{src_path}/cmake-build-local"):
        shutil.rmtree(f"{src_path}/cmake-build-local")

    # copy add items to temporary location
    if os.environ.get('READTHEDOCS') == 'True':
        shutil.copy(
            f"{src_path}/dearpygui/_dearpygui_RTD.py",
            f"{src_path}/output/dearpygui",
        )
    else:
        shutil.copy(
            f"{src_path}/dearpygui/dearpygui.py",
            f"{src_path}/output/dearpygui",
        )

    shutil.copy(f"{src_path}/dearpygui/demo.py", f"{src_path}/output/dearpygui")
    shutil.copy(
        f"{src_path}/dearpygui/experimental.py", f"{src_path}/output/dearpygui"
    )

    with open(f"{src_path}/output/dearpygui/__init__.py", 'w') as file:
        file.write(f"__version__='{version_number()}" + "'\n")

    if os.environ.get('READTHEDOCS') == 'True':

        os.rename(
            f"{src_path}/output/dearpygui/_dearpygui_RTD.py",
            f"{src_path}/output/dearpygui/dearpygui.py",
        )
        with open(f"{src_path}/output/dearpygui/_dearpygui.py", 'w') as newfile:
            with open(f"{src_path}/dearpygui/_dearpygui.pyi", 'r') as file:
        ...
        # copy add items to temporary location
        shutil.copy(
            f"{src_path}/dearpygui/_dearpygui.pyi",
            f"{src_path}/output/dearpygui",
        )
        if get_platform() == "Windows":
            shutil.copy(
                f"{src_path}/thirdparty/Microsoft/vcruntime140_1.dll",
                f"{src_path}/output/dearpygui",
            )
